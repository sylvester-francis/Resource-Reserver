...
import pyperclip  # For copying to clipboard (optional)
import requests
import typer
...
def mfa_setup():
    ...
    try:
        ...
        if "already enabled" in str(e):
            ...
            print("💡 Use [cyan]cli auth mfa disable[/cyan] first if you want to reset it")
        else:
...
def mfa_disable():
    ...
    try:
        result = client.mfa_disable(password)
...
def assign_role(
    ...
    try:
        result = client.assign_role(user_id, role_name)
        print(f"✅ [bold green]Role '{role_name}' assigned to user {user_id}[/bold green]")
...
def remove_role(
    ...
    try:
        result = client.remove_role(user_id, role_name)
...
def delete_oauth_client(
    ...
    try:
        result = client.delete_oauth_client(client_id)
        print(f"\n✅ [bold green]OAuth2 client deleted successfully[/bold green]")
        
    except requests.exceptions.HTTPError as e:
        if "404" in str(e) or "not found" in str(e):
            print("❌ OAuth2 client not found or you don't have permission to delete it")
        else:
