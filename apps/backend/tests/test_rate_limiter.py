...
class TestRateLimiterModule:
    ...
    def test_endpoint_limits_configured(self):
        ...
        assert ENDPOINT_LIMITS["/api/v1/token"] > 0  # Default is 30
...
class TestGetEndpointLimit:
    ...
    def test_get_specific_endpoint_limit(self):
        ...
        from app.core.rate_limiter import ENDPOINT_LIMITS, get_endpoint_limit

        # Should return configured limits
        assert get_endpoint_limit("/api/v1/token") == ENDPOINT_LIMITS["/api/v1/token"]
        assert (
            get_endpoint_limit("/api/v1/register")
            == ENDPOINT_LIMITS["/api/v1/register"]
        )
    ...
    def test_get_bulk_endpoint_limit(self):
        ...
        assert limit == ENDPOINT_LIMITS["/api/v1/bulk/"]
    ...
    def test_get_endpoint_limit_with_custom_limits(self):
        """Test getting endpoint limit with custom limit dictionary."""
        from app.core.rate_limiter import get_endpoint_limit

        custom_limits = {"/api/v1/test": 100, "/api/v1/custom/": 50}

        assert get_endpoint_limit("/api/v1/test", custom_limits) == 100
        assert get_endpoint_limit("/api/v1/custom/endpoint", custom_limits) == 50
        assert get_endpoint_limit("/api/v1/other", custom_limits) is None


class TestTestingModeLimits:
    """Tests for testing mode rate limits."""

    def test_testing_limits_higher_than_normal(self):
        """Test that testing mode limits are higher than normal limits."""
        from app.core.rate_limiter import (
            ENDPOINT_LIMITS,
            ENDPOINT_LIMITS_TESTING,
            TIER_LIMITS,
            TIER_LIMITS_TESTING,
            UserTier,
        )
        ...
        for tier in UserTier:
            assert TIER_LIMITS_TESTING[tier] >= TIER_LIMITS[tier]

        for endpoint in ENDPOINT_LIMITS:
            if endpoint in ENDPOINT_LIMITS_TESTING:
                assert ENDPOINT_LIMITS_TESTING[endpoint] >= ENDPOINT_LIMITS[endpoint]

    def test_testing_limits_configured(self):
        """Test that testing mode limits are properly configured."""
        from app.core.rate_limiter import TIER_LIMITS_TESTING, UserTier

        # All tiers should have testing limits
        for tier in UserTier:
            assert tier in TIER_LIMITS_TESTING
            assert TIER_LIMITS_TESTING[tier] > 0
