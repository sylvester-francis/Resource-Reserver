...
1. **Safari's strict enforcement**: Safari requires explicit cookie attributes, especially for localhost development
1. **Chromium browsers' leniency**: Chrome, Brave, Edge default to `sameSite: Lax` when not specified, masking the issue
...
1. Added `sameSite: 'Lax'` to the `username` cookie
...
1. Test login on Chrome/Brave to ensure no regression
1. Verify session persists across page refreshes
1. Test logout functionality
1. Verify cookies are cleared on logout
