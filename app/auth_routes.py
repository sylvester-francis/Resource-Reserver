...
from fastapi import APIRouter, Depends, HTTPException, Header, Form, status
...
def assign_role_to_user(
    ...
    if rbac.assign_role(request.user_id, request.role_name, db):
...
def remove_role_from_user(
    ...
    if rbac.remove_role(request.user_id, request.role_name, db):
...
def list_my_oauth_clients(
    ...
    clients = db.query(models.OAuth2Client).filter(
        models.OAuth2Client.owner_id == current_user.id
    ).all()
    return clients
...
@oauth_router.post("/introspect", response_model=auth_schemas.OAuth2TokenIntrospectionResponse)
